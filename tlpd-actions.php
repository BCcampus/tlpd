...
add_action(
	'init', function () {
		...
		if ( ! empty( $_REQUEST['em_ajax'] ) || ! empty( $_REQUEST['em_ajax_action'] ) ) {
			if ( isset( $_REQUEST['em_ajax_action'] ) && $_REQUEST['em_ajax_action'] === 'get_location' ) { //@codingStandardsIgnoreLine
				if ( isset( $_REQUEST['id'] ) ) {
					$EM_Location                        = new EM_Location( $_REQUEST['id'], 'location_id' ); //@codingStandardsIgnoreLine
			...
			if ( isset( $_REQUEST['em_ajax_action'] ) && $_REQUEST['em_ajax_action'] == 'delete_ticket' ) { //@codingStandardsIgnoreLine
				if ( isset( $_REQUEST['id'] ) ) {
					$EM_Ticket = new EM_Ticket( $_REQUEST['id'] ); //@codingStandardsIgnoreLine
			...
			if ( isset( $_REQUEST['query'] ) && $_REQUEST['query'] == 'GlobalMapData' ) { //@codingStandardsIgnoreLine
				$args           = [
					'eventful' => true,
					$_REQUEST, //@codingStandardsIgnoreLine
				];
			...
			if ( isset( $_REQUEST['ajaxCalendar'] ) && $_REQUEST['ajaxCalendar'] ) { //@codingStandardsIgnoreLine
				//FIXME if long events enabled originally, this won't show up on ajax call
				echo EM_Calendar::output( $_REQUEST, false ); //@codingStandardsIgnoreLine
				die();
			}
		}

		//Event Actions
		if ( ! empty( $_REQUEST['action'] ) && substr( $_REQUEST['action'], 0, 5 ) == 'event' ) { //@codingStandardsIgnoreLine
			//Load the event object, with saved event if requested
			if ( ! empty( $_REQUEST['event_id'] ) ) {
				$EM_Event = new EM_Event( $_REQUEST['event_id'] ); //@codingStandardsIgnoreLine
			} else {
				$EM_Event = new EM_Event();
			}
			//Save Event, only via BP or via [event_form]
			if ( $_REQUEST['action'] == 'event_save' && $EM_Event->can_manage( 'edit_events', 'edit_others_events' ) ) { //@codingStandardsIgnoreLine
				//Check Nonces
...
", $term
		...
		if ( ! empty( $_REQUEST['action'] ) && substr( $_REQUEST['action'], 0, 7 ) == 'booking' && ( is_user_logged_in() || ( $_REQUEST['action'] == 'booking_add' && get_option( 'dbem_bookings_anonymous' ) ) ) ) {
			...
			if ( $_REQUEST['action'] == 'booking_add' ) {
				...
				if ( ! $EM_Event->get_bookings()->has_booking( get_current_user_id() ) || get_option( 'dbem_bookings_double' ) ) {
					...
					$EM_Booking->tickets_bookings->booking = $EM_Ticket_Booking->booking = $EM_Booking; //@codingStandardsIgnoreLine
				...
				if ( $EM_Booking->can_manage( 'manage_bookings', 'manage_others_bookings' ) ) {
					...
					$wpdb->update(
						EM_BOOKINGS_TABLE, [
							'booking_meta' => serialize( $EM_Booking->booking_meta ), //@codingStandardsIgnoreLine
						], [
		...
		if ( ! empty( $_REQUEST['action'] ) && substr( $_REQUEST['action'], 0, 6 ) == 'search' ) {
			...
			if ( $_REQUEST['action'] == 'search_states' ) {
				...
				$results = $wpdb->get_col( 'SELECT DISTINCT location_state FROM ' . EM_LOCATIONS_TABLE . " WHERE location_state IS NOT NULL AND location_state != '' $cond ORDER BY location_state" ); //@codingStandardsIgnoreLine
			...
			if ( $_REQUEST['action'] == 'search_towns' ) {
				...
				$cond    = ( count( $conds ) > 0 ) ? 'AND ' . implode( ' AND ', $conds ) : '';
				$results = $wpdb->get_col( 'SELECT DISTINCT location_town FROM ' . EM_LOCATIONS_TABLE . " WHERE location_town IS NOT NULL AND location_town != '' $cond  ORDER BY location_town" ); //@codingStandardsIgnoreLine
				if ( $_REQUEST['return_html'] ) {
					//quick shortcut for quick html form manipulation
					ob_start();
					?>
			...
			if ( $_REQUEST['action'] == 'search_regions' ) {
				...
				$cond    = ( count( $conds ) > 0 ) ? 'AND ' . implode( ' AND ', $conds ) : '';
				$results = $wpdb->get_results( 'SELECT DISTINCT location_region AS value FROM ' . EM_LOCATIONS_TABLE . " WHERE location_region IS NOT NULL AND location_region != '' $cond  ORDER BY location_region" ); //@codingStandardsIgnoreLine
				if ( $_REQUEST['return_html'] ) {
					//quick shortcut for quick html form manipulation
					ob_start();
					?>
