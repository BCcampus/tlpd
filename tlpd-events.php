 ...
 */
add_filter(
	'the_content', function ( $page_content ) {
		...
		if ( ! post_password_required() && in_array( //@codingStandardsIgnoreLine
			$post->ID, [
				$events_page_id,
				$locations_page_id,
				$categories_page_id,
			...
			if ( empty( $content ) ) {
				...
				if ( $post->ID == $events_page_id && $events_page_id != 0 ) {
					if ( ! empty( $_REQUEST['calendar_day'] ) ) { //@codingStandardsIgnoreLine
						//Events for a specific day
						$args = EM_Events::get_post_search( array_merge( $args, $_REQUEST ) ); //@codingStandardsIgnoreLine
						em_locate_template(
						...
						if ( get_option( 'dbem_display_calendar_in_events_page' ) ) {
							...
							if ( ! empty( $_REQUEST['action'] ) && ( $_REQUEST['action'] == 'search_events' || $_REQUEST['action'] == 'search_events_grouped' ) ) { //@codingStandardsIgnoreLine
								$args = EM_Events::get_post_search( array_merge( $args, $_REQUEST ) ); //@codingStandardsIgnoreLine
							}
							if ( empty( $args['scope'] ) ) {
					...
					if ( EM_MS_GLOBAL && is_object( $EM_Location ) ) {
						...
						//Intercept search request, if defined
						if ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] == 'search_locations' ) { //@codingStandardsIgnoreLine
							$args = EM_Locations::get_post_search( array_merge( $args, $_REQUEST ) ); //@codingStandardsIgnoreLine
						}
